...
clb  = colorbar(ax(1)); clb.Label.String ='$\epsilon$ (rad)'; clb.Location  = 'eastoutside'; clb.Label.Interpreter = 'latex'      
...
xlabel(ax(1),''); ylabel(ax(1),'$\gamma$ (Estuary number)')
...
xlabel(ax(4),'X (Friction number)')
ylabel(ax(4),'$\gamma$ (Estuary number)')
...
lgd = legend(ax,'Location','NorthEast'); lgd.Interpreter = 'latex';

ax =  subplot(2,2,2); hold(ax,'on'); box(ax,'on'); grid(ax,'on');
set( ax, 'XScale' , 'log')
for ii = 1:size(d,1)
semilogx(ax,T,E_surge(ii,:)./1000,'LineStyle','-','Color',Colors(ii,:),'LineWidth',1.5,'DisplayName', ['$\eta$ = ' num2str(surge_lv(ii),2) ' (m)'])
end 
text(ax,0.05,0.9,  '(d)', 'Units', 'Normalized')
xlabel(ax,''); ylabel(ax,'E (km)')
lgd = legend(ax,'Location','NorthEast'); lgd.Interpreter = 'latex';
%%
